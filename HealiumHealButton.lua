...
function Healium_HealButton_OnEnter(frame, motion)
    ...
    if (frame.id) then
		Healium_DebugPrint("frame.id " .. frame.id)
		if (not Healium.ShowToolTips) then
			Healium_DebugPrint("no tooltip")
			return 
		end	
		Healium_DebugPrint("show tooltip")
		...
		Healium_DebugPrint("no frame.id")
