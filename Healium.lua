...
Healium_Debug = false
local AddonVersion = "|cFFff9900 1.0.2|r"
...
Healium_MaxClassSpells = 50 -- For now this is manually set to the max number of class specific spells in Healium_Spell.Name which currently is priest
...
function GetSpellID(spell)
	...
	--name, highestRank, icon, castTime, minRange, maxRange, spellID = GetSpellInfo(spell)
...
function Healium_UpdateSpells()
	for k, v in ipairs (Healium_Spell.Name) do
		Healium_DebugPrint("Update Spell " .. Healium_Spell.Name[k])
		Healium_Spell.ID[k] = GetSpellID(Healium_Spell.Name[k])
		if (Healium_Spell.ID[k]) then
			Healium_DebugPrint(" -- has id " .. Healium_Spell.ID[k])
...
function Healium_UpdateButtonIcons()
	...
	for i=1, Healium_MaxButtons, 1 do
		...
		-- Healium_DebugPrint("updating button " .. i .. " with texture " .. texture)
		for _, k in ipairs(Healium_Frames) do
...
--		end
		...
		if not id then
			for k=1, Healium_MaxClassSpells, 1 do
				if (spell == Healium_Spell.Name[k]) then
					...
					if id then
						Healium_DebugPrint("Healium_UpdateButtonSpells button " .. id)
					end
					break
				end
			end
		...
		for _,k in ipairs(Healium_Frames) do
			local button = k.buttons[i]
			if button then 
				-- Healium_DebugPrint("update button spell " .. id .. " to button " .. button)
